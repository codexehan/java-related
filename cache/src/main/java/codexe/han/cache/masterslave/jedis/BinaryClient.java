...
public class BinaryClient extends Connection {
  ...
  public void blpop(final int timeout, final byte[]... keys) {
    ...
    args.add(redis.clients.jedis.Protocol.toByteArray(timeout));
  ...
  public void zunionstore(final byte[] dstkey, final redis.clients.jedis.ZParams params, final byte[]... sets) {
    final List<byte[]> args = new ArrayList<byte[]>();
    args.add(dstkey);
    args.add(redis.clients.jedis.Protocol.toByteArray(sets.length));
  ...
  public void zinterstore(final byte[] dstkey, final byte[]... sets) {
    sendCommand(ZINTERSTORE, joinParameters(dstkey, redis.clients.jedis.Protocol.toByteArray(sets.length), sets));
  }
  ...
  public void scan(final byte[] cursor, final redis.clients.jedis.ScanParams params) {
    final List<byte[]> args = new ArrayList<byte[]>();
    args.add(cursor);
    args.addAll(params.getParams());
  ...
  public void hscan(final byte[] key, final byte[] cursor, final redis.clients.jedis.ScanParams params) {
    final List<byte[]> args = new ArrayList<byte[]>();
    args.add(key);
    args.add(cursor);
    args.addAll(params.getParams());
  ...
  public void sscan(final byte[] key, final byte[] cursor, final redis.clients.jedis.ScanParams params) {
    final List<byte[]> args = new ArrayList<byte[]>();
    args.add(key);
    args.add(cursor);
    args.addAll(params.getParams());
  ...
  public void geodist(final byte[] key, final byte[] member1, final byte[] member2, final redis.clients.jedis.GeoUnit unit) {
  ...
  public void georadius(final byte[] key, final double longitude, final double latitude, final double radius, final redis.clients.jedis.GeoUnit unit) {
    sendCommand(GEORADIUS, key, toByteArray(longitude), toByteArray(latitude), toByteArray(radius),
      unit.raw);
  }

  public void georadiusReadonly(final byte[] key, final double longitude, final double latitude, final double radius, final redis.clients.jedis.GeoUnit unit) {
  ...
  public void georadiusByMember(final byte[] key, final byte[] member, final double radius, final redis.clients.jedis.GeoUnit unit) {
    sendCommand(GEORADIUSBYMEMBER, key, member, toByteArray(radius), unit.raw);
  }

  public void georadiusByMemberReadonly(final byte[] key, final byte[] member, final double radius, final redis.clients.jedis.GeoUnit unit) {
  ...
  public void xadd(final byte[] key, final byte[] id, final Map<byte[], byte[]> hash, long maxLen, boolean approximateLength) {
	  ...
	  if(maxLen < Long.MAX_VALUE) {
	    ...
	    if(approximateLength) {
	      params[index++] = redis.clients.jedis.Protocol.BYTES_TILDE;
	    }
